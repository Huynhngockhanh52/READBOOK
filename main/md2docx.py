...
code_style = "Command"
def_style = "KC DEF"
para_styles = tuple(style_map.values()) + (norm_style, def_style)
...
def checkTpl(tpl_path: Path) -> None:
    ...
    try:
        code_obj = doc.styles[code_style]
    except KeyError:
        missing.append(code_style)
    else:
        valid = {WdStyle.CHARACTER, WdStyle.PARAGRAPH}
        if code_obj.type not in valid:
            missing.append(f"{code_style} (style type không hợp lệ)")
    if missing:
        names = ", ".join(missing)
...
def nodeText(node: dict[str, Any]) -> str:
    if str(node.get("type", "")) in {"linebreak", "softbreak"}:
        return "\n"
    raw = node.get("raw")
    ...
    for i, item in enumerate(items):
        ...
        for child in kids:
            ...
            if kind in {"block_text", "paragraph"}:
                ...
                addList(doc, child, depth + 1, style)
            else:
                addBlock(doc, child, depth, use_def=style == def_style)
        if first:
            addPara(doc, [], style=style, prefix=prefix, depth=depth)


def addCode(doc: Any, node: dict[str, Any], quote: bool = False) -> None:
    code_obj = doc.styles[code_style]
    if code_obj.type == WdStyle.PARAGRAPH:
        para = doc.add_paragraph(style=code_style)
        addRun(para, str(node.get("raw", "")))
    else:
        para = doc.add_paragraph(style=norm_style)
        addRun(para, str(node.get("raw", "")), is_code=True)
    if quote:
        setIndent(para, 0)
...
def addBlock(
    ...
    if kind == "heading":
        ...
        style = def_style if use_def else norm_style
        addPara(doc, kids, style=style, quote=quote)
    elif kind == "block_code":
        addCode(doc, node, quote)
    elif kind == "list":
        style = def_style if use_def else norm_style
        addList(doc, node, depth, style)
    elif kind == "block_quote":
        for child in kids:
            addBlock(doc, child, depth, False, True)
        ...
        addPara(
            doc,
            [{"type": "text", "raw": str(node["raw"])}],
            style=style,
            quote=quote,
        )
    elif kids:
        for child in kids:
            addBlock(doc, child, depth, quote, use_def)
